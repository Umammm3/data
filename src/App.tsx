...
export default function App() {
  ...
  const [moduleType, setModuleType] = useState<'photo' | 'document'>('photo');
  ...
  const addModule = async (e: FormEvent) => {
    e.preventDefault();
    if (!moduleTitle.trim() || !moduleFile) return;

    setIsUploading(true);
    try {
      const fileRef = ref(storage, `modules/${Date.now()}_${moduleFile.name}`);
      ...
      await addDoc(collection(db, 'modules'), newModuleData);
      setModuleTitle('');
      setModuleFile(null);
      // Reset file input manually
      const fileInput = document.getElementById('moduleFile') as HTMLInputElement;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="mb-12 text-center relative">
          <motion.div 
            ...
            className="inline-flex items-center justify-center p-4 bg-white border border-blue-500/20 text-blue-600 rounded-2xl mb-6 shadow-sm cursor-pointer active:scale-95 transition-all hover:border-blue-400 hover:shadow-md"
          >
            <Package size={40} />
          </motion.div>
          ...
          {role === 'admin' && (
            <motion.div 
              ...
              className="absolute top-0 right-0 glass-card px-4 py-2 rounded-xl text-[10px] font-bold border border-blue-500/20 flex items-center gap-2 tracking-widest text-blue-700"
            >
              <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
              SYSTEM ADMIN
              <button 
                onClick={() => setRole('guest')}
                className="ml-2 text-slate-400 hover:text-blue-600 transition-colors uppercase"
              >
                [EXIT]
              </button>
            </motion.div>
          ...
          >
            CORE<span className="text-blue-600">DATABASE</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            ...
            <div className="flex flex-col sm:flex-row gap-4 flex-1 max-w-2xl">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/5 transition-all text-sm font-mono tracking-wider uppercase placeholder:text-slate-400 text-slate-800"
                />
              </div>
              ...
              <div className="relative flex-1 sm:max-w-[200px]">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/5 transition-all text-sm font-mono text-slate-600"
                />
                {searchDate && (
                  <button 
                    ...
                    filteredItems.map((item) => (
                      ...
                      >
                        <td className="px-6 py-5">
                          <span className="font-mono font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg text-xs border border-blue-500/10">
                            {item.code}
                          </span>
                        </td>
                        <td className="px-6 py-5">
                ...
                filteredItems.map((item) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <span className="font-mono font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-lg text-[10px] border border-blue-500/10 uppercase">
                        {item.code}
                      </span>
                      {role === 'admin' && (
                    ...
                    {item.description && (
                      <div className="bg-slate-50 p-4 rounded-xl text-slate-500 text-xs italic border border-slate-100 flex items-start gap-3">
                        <Info size={14} className="mt-0.5 shrink-0 text-blue-500/30" />
                        {item.description}
                      </div>
                    )}
            ...
            >
              ...
              <form onSubmit={addModule} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Asset Type</label>
                    <select
                      value={moduleType}
                      onChange={(e) => setModuleType(e.target.value as 'photo' | 'document')}
                      className="w-full input-field appearance-none text-sm bg-white"
                    >
                      <option value="photo">Visual / Image</option>
                      <option value="document">Technical / Document</option>
                    </select>
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Source File</label>
                  <input
                    id="moduleFile"
                    type="file"
                    onChange={(e) => setModuleFile(e.target.files?.[0] || null)}
                    className="w-full text-xs text-slate-400 file:mr-6 file:py-2.5 file:px-6 file:rounded-xl file:border file:border-blue-500/10 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 file:transition-all file:cursor-pointer"
                    required
                  />
                </div>
                <button
                  ...
                  className="w-full py-4 neon-button flex items-center justify-center gap-3 uppercase text-xs tracking-[0.2em]"
                >
                  {isUploading ? (
              ...
              modules.map((module) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div className={`p-4 rounded-xl border ${module.type === 'photo' ? 'bg-blue-50 border-blue-100 text-blue-600' : 'bg-indigo-50 border-indigo-100 text-indigo-600'}`}>
                      {module.type === 'photo' ? <ImageIcon size={28} /> : <FileText size={28} />}
                    </div>
                    {role === 'admin' && (
                      <button
                  ...
                  <h3 className="font-bold text-slate-900 text-lg mb-1 line-clamp-1 uppercase tracking-tight">{module.title}</h3>
                  <p className="text-[10px] font-mono text-slate-400 mb-6 truncate uppercase tracking-widest">{module.fileName}</p>
                  ...
                  <div className="flex gap-3">
                    <a
                      ...
                      className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-100 text-slate-600 text-[10px] font-bold rounded-xl hover:bg-slate-200 transition-all uppercase tracking-widest"
                    >
                    ...
                    <a
                      ...
                      className="flex items-center justify-center p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition-all border border-blue-500/10"
                    >
                      <Download size={18} />
                    </a>
                  </div>
        ...
        {showLoginModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-md">
            ...
            >
              ...
              <div className="p-8">
                <div className="flex items-center justify-center w-20 h-20 bg-blue-50 text-blue-600 rounded-2xl mx-auto mb-8 border border-blue-500/10 shadow-sm">
                  <Package size={40} />
                </div>
                  ...
                  <div>
                    <input
                      ...
                      className={`w-full px-6 py-4 bg-white border rounded-2xl focus:outline-none focus:ring-4 transition-all text-center text-xl tracking-[0.5em] font-mono ${
                        loginError ? 'border-red-500 ring-red-500/5 text-red-600' : 'border-slate-200 focus:ring-blue-500/5 text-blue-600'
                      }`}
                    />
                    {loginError && (
